...
import { getFinanceGoalById } from "@/features/finance/queries";
...
export default async function FinanceGoalDetailPage({ params }: Props) {
  const { id } = await params;
  const session = (await getSession())!;
  const goal = await getFinanceGoalById(id, session.user.id);

  if (!goal) notFound();

  const progress = calculateGoalProgress(goal.currentValue, goal.targetValue);
  const isMonetary = goal.targetUnit?.toUpperCase() === "EUR";
